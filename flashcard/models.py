...
class Desafio(models.Model):
    ...
    def definir_status(self):
        ...
        if flashcards_respondidos.count() == self.quantidade_perguntas:
            acertos = flashcards_respondidos.filter(acertou=True).count()
            self.status = f"Concluido - {acertos}/{self.quantidade_perguntas} acertos"
        else:
            self.status = "Aguardando Respostas"
